...
GameSystem::GameSystem(SharedContext* l_context)
	...
	playingPokemons.push_back(new Pokemon("Carapuce", glm::ivec2(1, 1), &player
		, context->shaderManager, glm::vec3(0.0f, 0.0f, 1.0f)));
	playingPokemons.push_back(new Pokemon("Salamèche", glm::ivec2(1, 1), &player
		, context->shaderManager, glm::vec3(1.0f, 0.0f, 0.0f)));
	player.AddPokemon(playingPokemons[0], { 0,0 });
	player.AddPokemon(playingPokemons[1], { 2,2 });

	playingPokemons.push_back(new Pokemon("Bulbizarre", glm::ivec2(1, 1), &opponent
		, context->shaderManager, glm::vec3(0.0f, 1.0f, 0.0f)));
